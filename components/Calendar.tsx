...
const CalendarView: React.FC<CalendarProps> = ({ students, appointments, notes, lessonPlans, onSchedule, onUpdateAppointment, onQuickAddStudent, onStartSession }) => {
  ...
  const allEvents = useMemo(() => {
    ...
    return [...appts, ...standalone].sort((a, b) => a.sortDate.getTime() - b.sortDate.getTime());
  }, [appointments, notes]);

  const filteredEvents = useMemo(() => allEvents.filter(evt => {
  ...
  }), [allEvents, students, searchQuery, statusFilter, lessonPlanFilter]);
          ...
          return (
            <div key={d} onClick={() => handleOpenSchedule(dateStr)} className={`bg-white h-32 md:h-40 p-2 border-slate-100 cursor-pointer hover:bg-indigo-50/30 transition-all group relative ${isToday ? 'ring-2 ring-indigo-500 ring-inset z-10' : ''}`}>
              ...
              <div className="space-y-1 overflow-y-auto max-h-[80%] custom-scrollbar">{dayEvents.map(evt => {
                const s1 = students.find(s => s.id === evt.studentIds[0]); const time = new Date(evt.dateTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                return (<div key={evt.id} onClick={(e) => { e.stopPropagation(); handleOpenSchedule(dateStr, evt); }} className={`p-1.5 rounded-lg text-[9px] font-bold border truncate shadow-sm ${evt.status === 'completed' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-indigo-50 text-indigo-700 border-indigo-100'}`}><span className="opacity-60 mr-1">{time}</span> {s1?.name.split(' ')[0]}</div>);
              })}</div>
            </div>
          );
        })}
      ...
      {filteredEvents.map(evt => {
        ...
        return (
          <div key={evt.id} className="bg-white p-6 rounded-[2rem] border-2 border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-sm">
            ...
            <div className="flex gap-2 no-print"><button onClick={() => onStartSession(evt.studentIds, evt.id, (evt as any).lessonPlanId)} className="bg-indigo-700 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase shadow-lg">Start</button></div>
          </div>
        );
      })}
    </div>
      ...
      {isScheduleModalOpen && (
        <div className="fixed inset-0 z-[120] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 no-print">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[95vh]">
             <div className="bg-indigo-700 p-8 text-white flex items-center justify-between shrink-0"><div><h4 className="text-xl font-black">Schedule Session</h4><p className="text-[10px] font-black uppercase text-indigo-100">{selectedDateStr}</p></div><button onClick={() => setIsScheduleModalOpen(false)}><i className="fas fa-times text-xl"></i></button></div>
             <form onSubmit={handleScheduleSubmit} className="p-8 space-y-6 overflow-y-auto flex-1">
                <div className="max-h-48 overflow-y-auto border-2 border-slate-100 rounded-2xl p-2 space-y-1 bg-slate-50">{students.filter(s => s.isActive).map(s => (<div key={s.id} onClick={() => setFormData(p => ({ ...p, studentIds: p.studentIds.includes(s.id) ? p.studentIds.filter(id => id !== s.id) : [...p.studentIds, s.id] }))} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all border-2 ${formData.studentIds.includes(s.id) ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-transparent'}`}><div className={`w-6 h-6 rounded-md border-2 flex items-center justify-center ${formData.studentIds.includes(s.id) ? 'bg-indigo-600 border-indigo-600 text-white' : 'border-slate-300'}`}>{formData.studentIds.includes(s.id) && <i className="fas fa-check text-[10px]"></i>}</div><span className="text-sm font-bold">{s.name}</span></div>))}</div>
                <div className="grid grid-cols-2 gap-4"><div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Time</label><input type="time" className="w-full px-6 py-4 rounded-2xl border-2 border-slate-100 font-bold outline-none shadow-sm" value={formData.time} onChange={(e) => setFormData({...formData, time: e.target.value})} /></div><div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Duration</label><select className="w-full px-6 py-4 rounded-2xl border-2 border-slate-100 font-bold outline-none shadow-sm cursor-pointer" value={formData.duration} onChange={(e) => setFormData({...formData, duration: parseInt(e.target.value)})}>{[15, 30, 45, 60].map(m => <option key={m} value={m}>{m} mins</option>)}</select></div></div>
                <button type="submit" disabled={formData.studentIds.length === 0} className="w-full py-4 bg-indigo-700 text-white font-black rounded-2xl shadow-xl hover:bg-indigo-800 disabled:opacity-50">Confirm Schedule</button>
             </form>
          </div>
        </div>
      )}
